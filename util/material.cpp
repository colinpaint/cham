...
//}}}
//{{{
Image::Image (const uint8_t *buf, int width, int height, int channels, 
              const std::string &name, ColorSpace color_space)
    : name(name), width(width), height(height), channels(channels),
      img(buf, buf + width * height * channels), color_space(color_space) { }
//}}}
