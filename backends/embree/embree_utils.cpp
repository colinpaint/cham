...
namespace embree {
  ...
  Instance::Instance (RTCDevice &device, std::shared_ptr<TriangleMesh> &mesh,
                      const glm::mat4 &xfm, const std::vector<uint32_t> &material_ids)
      : handle (rtcNewGeometry(device, RTC_GEOMETRY_TYPE_INSTANCE)),
        mesh(mesh),
        object_to_world(xfm), world_to_object (glm::inverse (object_to_world)),
        material_ids(material_ids) {

    rtcSetGeometryInstancedScene (handle, mesh->handle());
    rtcSetGeometryTransform (handle, 0, RTC_FORMAT_FLOAT4X4_COLUMN_MAJOR, glm::value_ptr (object_to_world));
