...
void RenderOSPRay::set_scene (const Scene &in_scene) {
  ...
  for (const auto& mat : scene.materials) {
    ...
    // TODO: Seems like "specular" here means something really different and weird or is buggy
  ...
  {
    OSPData material_list = ospNewSharedData (materials.data(), OSP_MATERIAL, materials.size());
  ...
  for (const auto &inst : scene.instances) {
    ...
    OSPData stage = ospNewSharedData (geom_models.data(), OSP_GEOMETRIC_MODEL, geom_models.size());
